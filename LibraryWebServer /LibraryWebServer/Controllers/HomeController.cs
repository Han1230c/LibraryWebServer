...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin( string name, int cardnum )
        {
            ...
            var patron = db.Patrons.FirstOrDefault(p => p.Name == name && p.CardNum == (uint)cardnum);
            loginSuccessful = patron != null;
        ...
        public ActionResult AllTitles()
        {
            var query = from t in db.Titles
                        ...
                        from inv in inventory.DefaultIfEmpty()
                        join c in db.CheckedOut on inv.Serial equals c.Serial into checkedOut
                        from co in checkedOut.DefaultIfEmpty()
                        join p in db.Patrons on co.CardNum equals p.CardNum into patrons
                        from patron in patrons.DefaultIfEmpty()
                        select new
                        {
                            isbn = t.Isbn,
                            title = t.Title,
                            author = t.Author,
                            serial = (uint?)inv.Serial,
